...
README는 데이터 소비자가 데이터의 적합성을 판단하고 정확히 해석할 수 있도록 Dataset Card 문서로 작성합니다.
...
- [ ] 데이터의 적합성 판단과 해석에 필요한 정보가 설명되어 있다.
- [ ] 해당하지 않는 빈 항목과 데이터 소비자에게 불필요한 작업 절차가 없다.

## 방법

- `data/collection.json`과 실제 데이터를 바탕으로 저장소 루트의 `README.md`를 작성합니다.
- 데이터의 내용과 목적, 시간·공간 범위, 관측 단위, 구조, 변수와 단위, 품질과 한계, 출처와 라이선스 중 해당하는 정보를 중심으로 설명합니다.
- 데이터의 의미나 품질에 영향을 주는 수집·처리 내역은 소비자가 이해할 수 있는 수준으로 설명합니다.
...
- 공식 템플릿은 작성 항목을 검토하는 기준으로 사용하고, 해당 데이터와 관련 없거나 소비자의 판단·해석에 필요하지 않은 항목은 생략합니다.
