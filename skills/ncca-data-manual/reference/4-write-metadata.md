...
STAC 메타데이터는 데이터를 검색·활용하려는 사람이 데이터를 이해할 수 있도록 작성합니다.
...
- [ ] 데이터 소비자의 검색·적합성 판단·해석에 필요한 암묵지가 STAC 메타데이터에 반영되어 있다.
...
- 라이선스와 제공자의 역할, 원본 출처가 불분명한 경우 임의 작성 금지 및 필요한 근거 요청
- 뼈대의 빈 값이나 데이터 파일 참조 누락이 있는 경우 미완료 판단
- 파이프라인 실행 방법보다 데이터의 의미·범위·구조·품질·출처 우선 기록
