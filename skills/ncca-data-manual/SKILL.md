...
- 실행에 오래 걸리는 코드는 대표 데이터 일부로 먼저 실행해 정상 동작을 확인하세요.
- 오래 걸리는 코드에는 현재 단계와 진행률을 표시하세요.
- 오래 걸리는 명령어는 실행 명령어를 업무 담당자에게 제시하고 직접 실행해 달라고 요청하세요.
- STAC 메타데이터, Dataset Card, 코드 주석과 docstring을 작성하거나 검토할 때는 [테크니컬 라이팅](reference/technical-writing.md)을 따르세요.
